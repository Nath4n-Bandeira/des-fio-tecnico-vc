...
```
Voce tambem pode especificar o numero de nucleos logicos usados pelo Go com `-cpus`:

```powershell
go run . -size small -cpus 12
go run . -size medium -cpus 12
go run . -size large -cpus 12
go run . -size extra-large -cpus 12
```

Para processar todos, depois que todos os arquivos `.dat` estiverem disponiveis:
